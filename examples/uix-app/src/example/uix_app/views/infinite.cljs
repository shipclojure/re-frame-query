(ns example.uix-app.views.infinite
...
(defui feed-item [{:keys [item]}]
  (let [{:keys [id title body author created_at]} item]
...
(defui feed-stats [{:keys [data user]}]
  (let [{:keys [pages page-params has-next?]} data]
...
(defui user-switcher [{:keys [active-user]}]
  ($ :div {:style {:display "flex" :gap "0.5rem" :margin-bottom "1rem"}}
     (for [user ["alex" "maria"]]
       ($ :button {:key user
                   :class (if (= user active-user) "primary" "secondary")
...
(defui panel []
  (let [user (or (urf/use-subscribe [:ui/get :infinite/user]) "alex")
    ...
    ($ :div
       ...
       ;; --- User switcher ---
       ($ user-switcher {:active-user user})
       ...
       ;; --- Feed ---
       ($ :div.panel
          ($ :h3 (str "📰 " (str/capitalize user) "'s Feed")
             (when fetching?
               ($ :span {:style {:font-size "0.8rem" :color "#999" :margin-left "0.5rem"}}
                  "🔄 Refreshing…")))

          (when show-stats
            ($ feed-stats {:data data :user user}))
          ...
          (case status
            ...
            ($ :div
               (if (seq items)
                 ($ :div
                    (for [item items]
                      ($ feed-item {:key (:id item) :item item})))
