...
| `:infinite` | | Map with `{:initial-cursor val :get-next-cursor fn}` — enables infinite query mode. See [Infinite Queries](#infinite-queries). |
...
| `[::rfq/infinite-query k params]` | ✅ Yes | Full infinite query state (pages, cursors, has-next?) |
